...
Shader "Shader Forge/oy_Glass" {
    ...
    SubShader {
        ...
        Pass {
            ...
            #pragma target 3.0
			UNITY_DECLARE_SCREENSPACE_TEXTURE(_GrabTexture); //for Single Pass Instancing
            ...
            VertexOutput vert (VertexInput v) {
				...
				UNITY_SETUP_INSTANCE_ID(v); //for Single Pass Instancing
				UNITY_INITIALIZE_OUTPUT(VertexOutput, o); //for Single Pass Instancing
				UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(o); //for Single Pass Instancing
            ...
            float4 frag(VertexOutput i) : COLOR {
				UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i); //for Single Pass Instancing
                ...
                float4 sceneColor = UNITY_SAMPLE_SCREENSPACE_TEXTURE(_GrabTexture, sceneUVs); //for Single Pass Instancing
...
////// Specular:
                ...
                float3 diffuseColor = (_Albedo_Base_Color.rgb+(UNITY_SAMPLE_SCREENSPACE_TEXTURE( _GrabTexture, lerp( sceneUVs.rg, ((mul( float4(reflect(viewReflectDirection,i.normalDir),0), UNITY_MATRIX_V ).xyz.rgb.rg*pow(1.0-max(0,dot(_UseLensEffect_var, viewDirection)),_Distortion))+sceneUVs.rg), _UseDistortion )).rgb*_Incident_Color.rgb)); // Need this for specular when using metallic //for Single Pass Instancing
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 _NormalMap_var = UnpackNormal(tex2D(_NormalMap,TRANSFORM_TEX(i.uv0, _NormalMap)));
                float3 _UseLensEffect_var = lerp( _NormalMap_var.rgb, Colormapping_1( i.uv0.r , i.uv0.g , _LensCurve , _LensRadius , _LensOffsetX , _LensOffsetY ), _UseLensEffect );
                float3 diffColor = (_Albedo_Base_Color.rgb+(UNITY_SAMPLE_SCREENSPACE_TEXTURE( _GrabTexture, lerp( sceneUVs.rg, ((mul( float4(reflect(viewReflectDirection,i.normalDir),0), UNITY_MATRIX_V ).xyz.rgb.rg*pow(1.0-max(0,dot(_UseLensEffect_var, viewDirection)),_Distortion))+sceneUVs.rg), _UseDistortion )).rgb*_Incident_Color.rgb)); //for Single Pass Instancing
